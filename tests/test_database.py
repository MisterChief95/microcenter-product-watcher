...
def run_tests():
    ...
    success_1, product_id_1 = db.add_product_for_user(
        test_user_1,
        "https://www.microcenter.com/product/12345/test-product-1",
        "131"
    )
    ...
    success_2, product_id_2 = db.add_product_for_user(
        test_user_1,
        "https://www.microcenter.com/product/67890/test-product-2",
        "131"
    )
    ...
    if product_id_1 == product_id_3:
        print(f"  ✅ Deduplication working! Same product ID for both users")
    else:
    ...
    prev, curr = db.update_product_stock(
        product_id_1,
        True,
        "Test Product Title"
    )
    ...
    if notified_product and notified_product['notified']:
        print(f"  ✅ Notification status updated")
    else:
        print(f"  ❌ Notification status NOT updated")
    print()
    ...
    print(f"  Recent stock checks:")
    for row in cursor.fetchall():
        print(f"    - Product {row['product_id']}: {'In Stock' if row['in_stock'] else 'Out of Stock'} at {row['checked_at']}")
    conn.close()
    print()
