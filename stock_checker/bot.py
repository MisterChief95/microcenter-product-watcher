...
async def list_products(interaction: discord.Interaction):
    ...
    if not products:
        await interaction.response.send_message(
            ...
            "Use `/register` to start monitoring a product!",
            ephemeral=True
        )
        return
        ...
        for i, product in enumerate(products):
            ...
            options.append(
                discord.SelectOption(
                    label=f"{i+1}. {title}",
                    description=f"Store: {product['store_number']}",
                    value=str(product['id'])
                )
            )

        select = Select(
            placeholder="Choose a product to view history...",
            options=options,
